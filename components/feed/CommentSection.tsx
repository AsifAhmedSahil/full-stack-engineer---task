        ...
        <div style={{ flex: 1, position: "relative" }}>
          <input
            ...
            style={{
              ...
              color: "var(--color, #2D3748)",
            }}
          />
          <button
            type="button"
            disabled={submitting || (!value.trim() && !imagePreview)}
            onClick={onSubmit}
            style={{
              position: "absolute",
              right: 10,
          ...
          >
            ...
            >
              <path
                fill={value.trim() || imagePreview ? "#1890FF" : "#ccc"}
                fillRule="evenodd"
...
function ReplyItem({
  ...
  const toggleLike = async () => {
    const res = await fetch(`/api/replies/${reply.id}/like`, {
      method: "POST",
    });
    if (res.ok) {
      const data = await res.json();
